...
export const KenBurnsImage: React.FC<{ src: string | null; total: number; index: number; move?: KenMove; intensity?: KenIntensity; driftSeed?: string; effects?: EffectsProp }> = ({ src, total, index, move, intensity, driftSeed, effects }) => {
  const f = useCurrentFrame();
  const { width, height } = useVideoConfig();
  // 씬별 변주 — move 미지정이면 종전 그대로(짝수 줌인/홀수 줌아웃, 팬 4종 순환), 지정 시 push(훅 강줌+미세 회전) 등 확장 무브.
  const m = kenBurnsMove(f, total, index, move, intensity, { w: width / 2, h: height / 2 }, driftSeed);
  ...
  return <AbsoluteFill style={{ transform }}><Img src={staticFile(src)} style={{ width: '100%', height: '100%', objectFit: 'cover' }} {...(effects ? { effects } : {})} /></AbsoluteFill>;
};
