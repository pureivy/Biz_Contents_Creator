# AI 콘텐츠 스튜디오 (biz-contents-creator)
...
### 설치
```bash
git clone https://github.com/pureivy/Biz_Contents_Creator.git biz-contents-creator
cd biz-contents-creator
pnpm install
...
코드 기본은 업종 중립이다. 특정 업종의 낱말(예: 원예의 "나무"·"묘목"·"전정")이 필요하면 브랜드 설정(`brand.yaml`, 제작실 "브랜드" 탭)에 아래 7개 필드로 넣는다. 전부 선택이고, 비우면 범용 기본값이 쓰인다.

| 필드 | 무엇을 통제하는가 | 기본값(미설정 시) |
| --- | --- | --- |
| `subjectNoun` | 프롬프트의 "대상 ○○" 자리에 들어갈 소재 총칭 명사 | `소재` |
| `subjectTraits` | 이미지 앵커 지시문에 쓰는 소재의 시각 특징 축 | `형태·색·크기·재질` |
| `subjectStopwords` | 소재 이름으로 오면 사전을 오염시키는 업종 일반어 | 없음 |
| `subjectGenericTerms` | 총칭만인 주제 제한·소재 앵커 판정·계열 분류 제외에 쓰는 소재 총칭어 | 없음 |
| `keywordExamples` | 주제 제안 프롬프트에 넣는 검색어 예시(2~3어절) | `seedKeywords` 앞 3개 |
| `subjectIntentTerms` | 발굴 시드에서 업종 의도가 담긴 검색어를 고르는 기준 | 범용 의도어 |
| `subjectAnchorPattern` | 이름·유래형 주제에 소재가 명시됐는지 판정하는 정규식 | 소재 카탈로그 이름으로 판정 |
| `activityAxes` | 행위 축 동의어 묶음(첫 항이 표준형, 예: `[[전정, 가지치기], [물주기, 급수]]`) — 계열 쿨다운의 결정적 라벨 폴백·매칭 토큰 전개 | 행위 축 없음 |

예를 들어 나무를 취급하는 브랜드라면 `subjectGenericTerms`에 나무·묘목·유실수·조경수를, `subjectAnchorPattern`에 `[가-힣]{1,6}나무`를, `subjectNoun`에 식물을 넣는 식이다.
...
- **소재 사전** — 브랜드별 `data/species-<slug>.yaml`(범용 모드 `data/species.yaml`, 서식은 `assets/species.example.yaml`). 표에 있는 소재는 디렉터 LLM 의 기억 대신 학명·잎·꽃·열매 표를 이미지 앵커로 쓰고, 없으면 종전대로 LLM 이 쓴다(fail-open). 보관소에 딱지를 붙여 올리면 그 이름을 사전이 배운다(아는 소재의 별칭이면 `aliases` 에, 처음 보는 소재면 `auto: true` 항목으로 — 사람이 확인하면 `verified: true`).
...
- 소재 사전은 전역 `data/species.yaml` 이 아니라 **브랜드별** `data/species-<slug>.yaml` 이고, 파일이 없으면 첫 자동 축적 때 뼈대를 만든다. 업스트림의 수종 31종 데이터는 싣지 않고 `assets/species.example.yaml` 로 서식만 남겼다. 사전 판정 규칙 테스트는 `src/content/__fixtures__/species.yaml` 표본으로 돌고, 실제 사전은 있을 때만 무결성을 검사한다.
- 원예 낱말이 박혀 있던 로직(소재 앵커 정규식·사실 게이트 역할문·불용어·검색 의도어·검색어 예시·이미지 앵커 지시문)은 brand.yaml 업종 어휘 프로필로 뺐다 — 코드 기본은 업종 중립.
