 ...
 * 활용해서 자율런을 돌려줘") — 주제를 고를 때 '어느 수종을 먼저 볼지'의 근거.
 *
 * 왜 이 신호가 값진가. 이 채널의 09-02 조회 급감 진단은 "쇼츠 피드가 초기 배분을 거뒀다"
 * 였고, 109편이 이어가지 못한 공통점은 생성 이미지로만 만든 편들이라는 것이다. 사장님이
 * 직접 찍은 밭 영상은 이 채널이 가진 유일한 차별 자산이다. 창고에 쌓아 두면 값이 0이다.
 *
 * ⚠ 이건 게이트가 아니라 동점 처리다.
 ...
 *   - 발행 전에 지운 편(purge)은 폴더가 사라져서, 그 편만 쓴 수종은 '미사용'으로 되돌아온다.
 *     되돌아온 수종을 한 번 더 다루는 것은 무해해서 그대로 둔다.
 *   - 유튜브·인스타 두 편이 같은 클립을 물려받으므로 '몇 번 썼나'는 세지 않는다. 썼나/안 썼나만 본다.
 *   - 사전이 모르는 딱지는 '미사용'으로 친다. 사장님이 직접 적은 이름이 유일한 근거이고,
...
export interface StockRow {
  /** 사장님이 적은 딱지 그대로 — 프롬프트에 이 이름으로 나간다. */
  readonly species: string;
...
/**
 * 아직 안 쓴 소재를 수종별로 묶는다(순수).
 *
 * @param items 보관소 목록
 * @param used  이미 실촬영이 들어간 수종의 표준명 집합
...
export function unusedStock(
  ...
  for (const m of items) {
    if (m.kind !== 'video') continue; // 사장님이 말한 것은 '영상 자료'다
 ...
 *
 * 문구가 이 함수의 전부다. "이 수종을 다뤄라"가 아니라 "다른 조건이 비슷하면 이쪽을
...
export function stockBlock(rows: readonly StockRow[]): string {
  ...
  return [
    '[실촬영 재고 — 아직 화면에 안 나간 사장님 촬영본이 있는 수종]',
    names.join(', ') + (rest > 0 ? ` 외 ${rest}종` : ''),
    '',
    '이 수종으로 주제를 잡으면 생성 이미지 대신 실제 밭 영상이 화면에 들어간다.',
    '이 채널이 가진 유일한 차별 자산이고, 창고에 두면 값이 0이다.',
    '',
    '⚠ 다만 이건 동점 처리다. 계절 시의성과 검색 수요가 먼저다 — 지금 심을 때가 아닌',
    '나무를 재고가 있다는 이유로 고르지 마라. 앞의 조건들이 비슷할 때 이 목록에 있는',
    '수종을 먼저 골라라. 목록에 마땅한 것이 없으면 무시하고 평소대로 골라라.',
  ].join('\n');
}

/**
 * 이미 실촬영이 들어간 수종을 읽는다(부수효과 — 숏폼 폴더를 훑는다).
 *
 * 근거는 `clips/user_*.mp4` 의 존재다. applyUserSegments 가 사장님 영상을 이 이름으로
