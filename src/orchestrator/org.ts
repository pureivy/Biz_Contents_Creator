...
const buildBlogBodyGuide = (variety: boolean): string =>
  ...
  '- 결론 의무(통설 진단 허용): 관찰·구별법을 다뤘으면 각 갈래마다 그것이 대개 무엇을 뜻하는지와 오늘 할 행동을 반드시 말하라. 업종 통설 수준의 인과는 "대개/흔히"를 붙여 단정해도 된다("이런 증상은 대개 관리 쪽 문제입니다") — 이는 겪지 않은 사건 날조와 다르다. 근거가 정말 갈리는 것만 유보하고, 글의 결론을 다음 편 예고로 통째 미루지 마라.\n' +
  ...
  '- 키워드 계층 분리: 핵심 키워드의 정확 표기는 첫 문단 1회·소제목 1곳까지만 — 그 밖에는 조사·어순을 바꾼 변형("겨울에 타이어를 고를 때는")으로 녹여라. 네이버는 형태소 분석이라 변형이 검색 손실을 만들지 않고, 정확 표기 반복·억지 관형절("한파 직전에 적용하는 겨울 타이어 고르는 법은," 식)은 과최적화로 오히려 해롭다.\n' +
...
export function buildGenreGuide(axes: readonly string[], teaser?: boolean): string {
  ...
  return `[글의 성격] 이 블로그가 한 가지 틀로만 반복되지 않게, 다음 축을 골고루 쓴다 — ${axes.map((x: string, i: number) => `${i + 1}) ${x}`).join(' ')}. `
    ...
    + '단, 계절 변화처럼 대상이 스스로 겪는 일을 다루는 관찰·이야기 글은 예외다 — 작업 지시로 억지 마무리하지 말고, 언제 어디서 무엇이 어떻게 보이는지 구체적인 관찰 포인트로 끝내도 좋다. '
