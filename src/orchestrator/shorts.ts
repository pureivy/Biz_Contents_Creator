...
import { lexiconGuide, subjectNoun, subjectTraits } from '../content/brand';
...
async function fitShortsPlanToDuration(io: JobIO, plan: Plan, budgetCharsOverride?: number): Promise<{ plan: Plan; compressed: boolean; beforeChars: number; afterChars: number; keptScenes: number[] }> {
  ...
  const j = await microJSON<{ scenes?: Array<{ index?: unknown; narration?: unknown } | null> }>(
    ...
    [
      ...
      '규칙: 사실·수치·결론과 씬별 "무엇을+왜"는 유지하고 군더더기·중복 표현만 깎아라. 조사·주어를 떨어낸 초압축은 감량이 아니라 훼손이다("이 시기 기본형 고르면" 금지 → "이 시기엔 기본형을 고르면"). 구어체 존댓말·숫자 한글 낭독 등 원문 문체 유지, 이모지 금지. 유보어("대개/흔히/보통/~일 수 있다/~봐요/가능성")는 군더더기가 아니다 — 남겨라. 원문 결론의 방향("미루라/하지 마라")을 뒤집지 마라.',
...
async function planShorts(io: JobIO, topic: string, keyword: string | undefined, sourceBody: string | undefined, n: number, priorCoverage = '', titleTypes: string[] = ['정보형', '후킹형', '질문형'], sourceFlagged: string[] = [], writerRole = 'shorts_writer', recentMainTitles: string[] = []): Promise<Plan | null> {
  ...
  const user = [
    ...
    '시청자는 원문 블로그를 본 적이 없다 — 영상 단독으로 완결되게 써라: 핵심 주제어(대상명·행위명 등)를 훅 또는 씬2에서 반드시 명시하고, 지시어·축약어("짝"·"그 방법"·"두 그루" 류)는 앞 씬에서 뜻을 세운 뒤에만 사용하라. 단, 명시는 검색 표기 그대로일 필요 없다 — 키워드 명사구를 통째로 주어 자리에 세우지 말고("겨울 타이어 고르는 법은 ~로 정해집니다" 식 금지, 검색용 문장 티가 난다) 조사·어순을 바꿔 말이 되는 꼴로 녹여라("겨울에 타이어를 고를 땐 가격보다 규격부터 봐요"). 검색 표기 그대로의 노출은 title·titles·description·해시태그·screenText 가 담당한다. 단 씬2(전제)의 정의 한 문장은 예외 — 뒤 씬에서 쓸 기준·개념(예: "두 선")은 씬2에서 이름 붙여 뜻을 세워라. 어순·조사는 바꿔도 되지만 정의 자체를 얼버무리지 마라. 훅에서 감춘 대상도 씬2에서 반드시 밝힌다.',
    ...
    '훅 첫 문장을 키워드·소재명·상품명 낭독으로 열지 마라 — 첫 2초(약 12자)가 스와이프 이탈을 결정한다. 긴장·장면·숫자·상황 지목으로 열고, 키워드 명시는 훅의 뒷문장 또는 씬2에서 하라.',
    ...
    '낭독 명료성: 소리로만 듣고 되돌릴 수 없는 매체다 — 동음이의 전문어는 일상어로 풀어 말하고(업종 용어를 처음 듣는 사람도 알아듣게), 주어·비교 대상을 떨어낸 조각문 금지("밀리미터보다 옆 가지와 비교해요" 식 초압축 — 실측). 내레이션에서 언급하는 개수("다섯 곳")는 실제로 그 수만큼 세어 줘야 한다.',
    ...
    '대본 전체 1~2곳에 사람 말의 결을 넣어라 — 짧은 제동("잠깐,"), 정정·양보("아, 소형은 예외예요"), 판단의 표명("겉보다 규격표부터 봐요" — "나는/저는" 같은 1인칭 주어는 쓰지 마라, 주어 없이도 시점이 전달된다. 사용자 확정 2026-08-12). 겪지 않은 사건·일화를 지어내는 것은 금지(판단·관점만). 정보 밀도를 깎는 잡담도 금지. 판단 문장은 근거·이유 문장을 밀어내고 그 자리를 차지하게 하지 마라 — 근거에 덧붙이는 한 줄이다. "X가 아니라 Y" 재정의 문형은 대본 전체 1회 이하 — 앞 편들이 이미 상투로 만든 틀이다.',
    ...
    `압축 안전선: ${capSec}초에 맞추더라도 조사·주어는 남겨라("이 시기 기본형 고르면" 금지 → "이 시기엔 기본형을 고르면"). 앞 문장의 주어가 바뀌면 새 주어를 밝혀라. screenText 에 "정의·구분법·요약·정리" 같은 대본용 딱지를 쓰지 마라 — 화면에는 독자에게 하는 말만.`,
    ...
    '  [먼저 할 일] 대본을 쓰기 전에 원문(블로그 초안)에서 수치와 나열을 뽑아라. 실용 글에는 대개 둘 다 있다 — 수치는 크기·양·횟수·기간·온도·비율·가격, 나열은 준비물·순서·조건·종류. 뽑은 것이 있으면 그 내용을 담은 본문 씬에 아래 kind 를 붙인다.',
    ...
    'description(유튜브 설명 — 인스타 캡션에도 그대로 복제된다)은 요약투로 끝내지 마라 — "정리했습니다/담았어요/알아봅니다/알아보세요/살펴봅니다/소개합니다" 금지. "결론 한 줄 + 조건 한 줄" 꼴로 써라(예: "겉만 보고 고르면 두 달 안에 후회합니다. 규격표의 두 줄부터 보세요.").',
    ...
    'compare 씬에는 bad·good 를 반드시 채워라 — kind 만 보내면 화면에 아무것도 안 나온다(한쪽이라도 비면 통째로 무시된다). bad=하지 말 것·틀린 쪽, good=할 것·맞는 쪽. label 은 대상(14자 이내), note 는 그 이유나 결과(18자 이내). 예: bad={"label":"겉만 보고 구매","note":"두 달 안에 후회해요"} good={"label":"규격표 먼저 확인","note":"오래 씁니다"}. 내레이션이 말한 대비를 화면에 옮기는 것이지 새 내용을 지어내는 게 아니다 — 원문에 없는 대비는 만들지 마라.',
...
async function designScenes(io: JobIO, topic: string, plan: Plan, keyword?: string): Promise<{ preset: string; style: string; prompts: string[]; subject?: string; subjectLatin?: string; species?: import('../content/species').Species }> {
  const user = [
    ...
    `대상 ${subjectNoun()} 고정(가장 중요): 이 영상의 대상을 두 칸으로 적어라. subject_latin 에는 학명(속명+종소명, 예: "Nandina domestica") 또는 식물이 아니면 영문 표준명, subject 에는 한글 이름과 그림으로 구별되는 특징을 한 줄로(${subjectTraits()}).`,
    '특징은 정확해야 한다 — 원문(블로그 초안)에 그 대상의 생김새 묘사가 있으면 그것을 우선하고, 확신이 없으면 특징을 짧게만 쓰되 학명(또는 영문 표준명)은 반드시 채워라 — 이름 하나가 애매한 산문보다 정확하다.',
    '학명·이름을 모르는 소재(도구·배경·일반 풍경)면 두 칸을 모두 비워라. 지어내지 마라.',
    '그리고 모든 씬 묘사(prompt)에 그 대상이 무엇인지 매번 명시하라 — 총칭으로 뭉뚱그리지 마라. 화면에 대상이 안 나오는 씬(도구·배경만)은 예외다.',
  ...
  if (known) {
    console.log('[숏폼]', `소재 사전 적중 — ${known.name} (${known.latin})${known.verified ? '' : ' · 형태 묘사 미검토'}`);
    ...
    if (nm && appendSpecies({ name: nm, latin: subjectLatin, leaf: subject })) {
      console.log('[숏폼]', `소재 사전 추가 — ${nm} (${subjectLatin}) · 검토 필요(auto)`);
    }
  }
  if (subject || subjectLatin) {
    console.log('[숏폼]', `소재 앵커 — ${subjectLatin ? `${subjectLatin} · ` : '학명 없음 · '}${subject || '(특징 없음)'}`);
    if (!subjectLatin) console.log('[숏폼]', '학명 미기재 — 한글 이름만으로는 이미지 모델이 종을 자주 틀린다');
  } else if (keyword) {
    console.log('[숏폼]', `소재 앵커 없음 — 디렉터가 '${keyword}' 겉모습을 못 적었다(화면 대상이 틀릴 수 있음)`);
  }
 ...
 * 숏폼 씬 이미지(gpt-image-2, 세로 9:16) 프롬프트 조립 — shorts-gen 의 스타일 일관성(전 씬 공통 앵커
 * 반복) + 시네마토그래피(구도·조명·씬 변주). 자막·내레이션은 렌더러가 얹으므로
 * 이미지엔 글자를 넣지 않는다(오타 원천 차단). 생성 없이 프롬프트 검증 가능 — 다만 앵커 문구의
 * 업종 낱말은 브랜드 설정(subjectNoun·subjectTraits)에서 온다.
 *
...
export function buildSceneImagePrompt(a: {
  ...
  p.push('[도구] 가위·톱·삽처럼 날·축·손잡이가 있는 도구는 화면에 넣지 마라 — 구조가 어긋나게 그려진다. 손은 대상을 짚거나 가리키는 정도로만 쓴다.');
  if (!seed) { // 종전 동작 보존
...
export async function runShortsJob(
  ...
  try {
    ...
    const qa = await qaSceneImages({
      ...
      ...(genRefs.length ? { refImages: genRefs } : {}),
      ...(design.subject ? { subject: design.subject } : {}),
      ...(design.subjectLatin ? { subjectLatin: design.subjectLatin } : {}), // 소재 오식별 검사
