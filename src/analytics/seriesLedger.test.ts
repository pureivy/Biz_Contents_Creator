...
import { computeSeriesScores, gateForLabels, fillLabelsFromKnown, normSpecies, SERIES_SOFT, SERIES_HARD_COMBO, SERIES_HARD_SINGLE } from './seriesLedger';
...
describe('computeSeriesScores — 지수 감쇠 점수(반감기 3.5일, 순수)', () => {
  it('오늘 1.0, 3.5일 전 ≈0.5로 감쇠하고 소재·행위·조합을 각각 집계한다', () => {
...
describe('gateForLabels — 조합 엄격·단독 느슨·소프트 2단계(순수)', () => {
  ...
  it('조합은 이틀 2편이면 하드, 같은 소재의 새 조합은 소프트로만 걸린다', () => {
    const s = computeSeriesScores([
    ...
    // 같은 소재의 새 조합: 감쇠 점수 ~1.5 < 2.4(단독 하드)라 종전엔 소프트였지만, 3일 바닥선(2026-08-27
    // 사용자: 며칠 만의 같은 소재 재등장 금지)이 먼저 걸려 하드. 3일이 지나면 종전대로 소프트.
    const newCombo = gateForLabels({ species: '포도', activity: '월동' }, s);
    expect(newCombo).toMatchObject({ level: 'hard', key: '포도', why: '소재 3일 2편' });
  ...
  it('N3 소재 바닥선 — 같은 소재가 3일 안에 1편이라도 있으면 하드(2026-08-27 올리브 3일 만 재등장)', () => {
    ...
    expect(gateForLabels({ species: '올리브나무', activity: '화분' }, sc)).toMatchObject({ level: 'hard', key: '올리브나무', why: '소재 3일 1편' });
...
describe('normSpecies — 총칭 접미 절단(순수, 총칭어는 브랜드 설정이 준다)', () => {
  const GEN = ['나무', '묘목', '유실수', '조경수', '정원수', '과실수', '과실나무', '식물', '수목'];
  it('어간이 두 글자 이상이면 총칭 접미를 뗀다', () => {
    expect(normSpecies('배롱나무', GEN)).toBe('배롱');
    expect(normSpecies('블루베리묘목', GEN)).toBe('블루베리');
  });
  it('단음절 어간은 원형을 지킨다 — 스트립하면 키가 소멸한다', () => {
    expect(normSpecies('감나무', GEN)).toBe('감나무');
    expect(normSpecies('배나무', GEN)).toBe('배나무');
  });
  it('총칭·범주어와 1자 이름은 null', () => {
    expect(normSpecies('과실나무', GEN)).toBeNull();
    expect(normSpecies('묘목', GEN)).toBeNull();
    expect(normSpecies('감', GEN)).toBeNull();       // 1자 순수형은 승격하지 않는다(키 공간 오염 방지)
    expect(normSpecies(null, GEN)).toBeNull();
    expect(normSpecies('null', GEN)).toBeNull();
  });
  it('총칭어 미설정(범용 브랜드)이면 절단·배제 없이 원형', () => {
    expect(normSpecies('배롱나무', [])).toBe('배롱나무');
    expect(normSpecies('묘목', [])).toBe('묘목');
  });
});
